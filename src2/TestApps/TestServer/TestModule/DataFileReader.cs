...
namespace TestServer.TestModule
{
	...
	public class DataFileReader
	{
		...
		public void CreateData(ArchiveItem item)
		{
			...
			DataGenerator generator = new DataGenerator(null);
		...
		public void LoadHistoryData(ISystemContext context, ArchiveItem item)
		{
			using(StreamReader reader = item.OpenArchive())
			{
				...
				reader.ReadLine();
				item.DataSet = LoadData(context, reader);
			}
		...
		/// </summary>
		private DataSet LoadData(ISystemContext context, StreamReader reader)
		{
		...
		private bool ExtractField(int lineCount, ref string line, out int value)
		{
			...
			{
			    Utils.Trace(Utils.TraceMasks.Error, "Quickstarts.HistoricalAccessServer.DataFileReader.ExtractField", string.Format("PARSE ERROR [Line:{0}] - '{1}': {2}", lineCount, field, e.Message));
                return false;
			}
