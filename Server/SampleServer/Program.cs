    ...
    class Program
    {
        ...
        private static async void StartServer()
        {
            ...
            try
            {
                ...
                // Load the Serilog configuration and set the Serilog logging service
                // ApplicationConfiguration appConf = defaultConfiguration.ApplicationConfiguration;
                ApplicationConfiguration appConf = await ConfigUtils.GetApplicationConfiguration(configurationFile, true, ApplicationType.Server).ConfigureAwait(false);
        ...
        private static async Task<ApplicationConfigurationBuilderEx> LoadDefaultConfiguration(ICertificatePasswordProvider certificatePasswordProvider = null)
        {
            ...
            await applicationConfigurationBuilder
                ...
                .AsServer(new string[] { "opc.tcp://localhost:61510/SampleServer" })
                    ...
                    .SetMaxDurableSubscriptionLifetime(1)
                    .SetMaxDurableNotificationQueueSize(10000)
                ...
                .AddExtension<ServerDurableConfiguration>(new XmlQualifiedName("ServerDurableConfiguration"),
                    new ServerDurableConfiguration(){ DurableSubscriptionsPath = "DurableSubscriptionsPath" })
                .SetTraceMasks(1)
        ...
        private static void PrintSessionList(SampleServer sampleServer)
        {
            ...
            {
                Console.WriteLine("\nSessions list:");
                foreach (Opc.Ua.Server.Session session in sessions)
                {
                    var sessionSubscriptions = subscriptions.Where(sub => sub.SessionId == session.Id).ToList();
                    List<(NodeId SubscriptionId, int MonitoredItemCount, bool IsDurable)> kSubscriptionMonitoredItems = GetSessionMonitoredItems(sessionSubscriptions);
                    ...
                    line.AppendFormat(";Subscriptions count:{0}", session.SessionDiagnostics.CurrentSubscriptionsCount);
                    foreach (var item in kSubscriptionMonitoredItems)
                    {
                        line.Append($"\n\t " +
                            $"Subscription ID: {item.SubscriptionId}, " +
                            $"IsDurable: {item.IsDurable}, " +
                            $"Monitored Items: {item.MonitoredItemCount}");
                    }
        ...
        private static List<(NodeId SubscriptionId, int MonitoredItemCount, bool IsDurable)>
            GetSessionMonitoredItems(IList<Opc.Ua.Server.Subscription> subscriptions)
        {
            var subscriptionInfo = new List<(NodeId SubscriptionId, int MonitoredItemCount, bool IsDurable)>();
            if (subscriptions?.Count == 0)
            {
                return new List<(NodeId, int, bool)>();
            }

            subscriptionInfo = subscriptions
                .Where(sub => sub?.Session != null)
                .GroupBy(sub => sub.Id)
                .Select(group => (
                    SubscriptionId: (NodeId)group.Key,
                    MonitoredItemCount: group.First().MonitoredItemCount,
                    IsDurable: group.First().IsDurable
                ))
                .ToList();
